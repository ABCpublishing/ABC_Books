...
async function fetchAllBooks() {
    ...
    if (stored) {
        try {
            ...
            if (now - parsed.timestamp < PERSISTENT_CACHE_TTL) {
                ...
                console.log(`ΓÜí Loaded ${parsed.books.length} books from persistent cache`);
                return _cachedBooks;
            }
        } catch (e) {
    ...
    _fetchPromise = (async () => {
        try {
            console.log('≡ƒîÉ Fetching fresh book data from API...');
            ...
            if (data && data.books) {
                ...
                console.log(`Γ£à Loaded ${_cachedBooks.length} books from database`);
                return _cachedBooks;
            }
            return [];
        } catch (error) {
            console.warn('ΓÜá∩╕Å Failed to fetch fresh books:', error.message);
            return _cachedBooks || [];
        } finally {
        ...
        <div class="book-card" data-book-id="${book.id}">
            <div class="book-cover" onclick="viewBookDetail('${book.id}')" style="cursor: pointer;">
                ...
                <div class="book-overlay">
                    ...
                    <div class="overlay-price">
                        <span class="current">Γé╣${book.price}</span>
                        ${book.originalPrice && book.originalPrice > book.price ? `<span class="original">Γé╣${book.originalPrice}</span>` : ''}
            ...
            <div class="book-info" onclick="viewBookDetail('${book.id}')" style="cursor: pointer;">
                ...
                <div class="book-price">
                    <span class="price-current">Γé╣${book.price}</span>
                    ${book.originalPrice && book.originalPrice > book.price ? `<span class="price-original">Γé╣${book.originalPrice}</span>` : ''}
                </div>
            </div>
        </div>
    `;
...
function viewBookDetail(bookId) {
    ...
    console.log(`≡ƒÜÇ Navigating to ${targetUrl} (isInsidePages: ${isInsidePages})`);
...
async function addToCartCard(bookId, bookData) {
    ...
    if (!isLoggedIn) {
        // Γ£à SAVE PENDING ACTION for auto-continue after login
